...
export default function PriceAnalysisCard({ data }: PriceAnalysisCardProps) {
  ...
  // Find the cheapest month (lowest price) from ALL data including estimates
  const cheapestMonth = histogram.reduce((min, current) => 
    current.avg_price < min.avg_price ? current : min
  , histogram[0])
  const bestMonthAbbr = cheapestMonth ? cheapestMonth.month.split(" ")[0] : null
